...
void usage()
{
	fprintf(stdout, "Usage: satdecap -i TAP_IFACE -l LOCAL_ADDR_PORT -r REMOTE_ADDR_PORT\n");
	fprintf(stdout, "                [-p PAYLOAD_LEN]\n");
	fprintf(stdout, "                [-b BUFFER_LEN]\n");
	fprintf(stdout, "                [-t READ_TIMEOUT]\n");
	fprintf(stdout, "                [-h]\n");
	fprintf(stdout, "\n    Required arguments\n");
	fprintf(stdout, "        TAP_IFACE         the TAP interface which forwards outcoming IP packets\n");
	fprintf(stdout, "        LOCAL_ADDR_PORT   the address and port to use as source of UDP tunnel (format: \"ADDRESS:PORT\")\n");
	fprintf(stdout, "        REMOTE_ADDR_PORT  the address and port to use as destination of UDP tunnel (format: \"ADDRESS:PORT\")\n");
	fprintf(stdout, "\n    Optional arguments\n");
	fprintf(stdout, "        PAYLOAD_LEN       the max size (bytes) of incoming payload from the UDP tunnel (default: %u)\n", DEFAULT_PAYLOAD_LENGTH);
	fprintf(stdout, "        BUFFER_LEN        the max size (bytes) of outcoming IP packet (default: %u))\n", DEFAULT_BUFFER_LENGTH);
	fprintf(stdout, "        READ_TIMEOUT      the timeout (ms) to read incoming payload from the UDP tunnel (default: %u)\n", DEFAULT_READ_TIMEOUT);
}

/**
 * Parse decapsulation tunnel parameters
 *
 * Return 0 on success, 1 when help is required, -1 on error
 */
int parse_arguments(int argc, char** argv, struct process_decap_params* params)
{
	...
	while((flags & error_flag) == 0 && (flags & help_flag) == 0 && (c = getopt(argc, argv, "hi:l:r:p:b:q:t:")) != -1)
	{
		switch(c)
		{
			case 'h':
			...
			{
				fprintf(stderr, "Invalid TAP interface \"%s\": must be an existing TAP interface\n", optarg);
				flags |= error_flag;
				break;
			}
			...
			{
				fprintf(stderr, "Invalid local address and port \"%s\" (format: \"ADDRESS:PORT\")\n", optarg);
				flags |= error_flag;
				break;
			}
			flags |= local_flag;
			...
			{
				fprintf(stderr, "Invalid remote address and port \"%s\" (format: \"ADDRESS:PORT\")\n", optarg);
				flags |= error_flag;
				break;
			}
			flags |= remote_flag;
			...
			{
				fprintf(stderr, "Invalid payload length \"%s\": the value must be an unsigned int in bytes\n", optarg);
				flags |= error_flag;
				break;
			}
			...
			if(parse_unsigned_long(optarg, &val) != 0 || val > UINT_MAX)
			{
				fprintf(stderr, "Invalid buffer length \"%s\": the value must be an unsigned long in bytes\n", optarg);
				flags |= error_flag;
				break;
			}
			...
			{
				fprintf(stderr, "Invalid reading timeout \"%s\": the value must be an unsigned long in milliseconds\n", optarg);
				flags |= error_flag;
				break;
			}
 ...
 * Main function
 *
 * Return 0 on success, -1 on arguments error, -2 on initialization error, -3 otherwise
 */
int main(int argc, char** argv)
{
	struct process_decap_params params;
	int ret;
